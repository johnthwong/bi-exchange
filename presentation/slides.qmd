...
![](img/edgeworth_axtell.png){width=60% fig-align="center"} 

\pause

-   What if we increase the number of agents to three? Or four? Or any arbitrary *A*? \pause
-   Or what if we increase the number of goods to *N*?
...
\ \ \ \ \ \ FOR n in 1:(N-1): \\
\ \ \ \ \ \ \ \ FOR q in 1:(N-n): \\
\ \ \ \ \ \ \ \ \ \ agent with higher MRS(n, n+q) offers good n \\
\ \ \ \ \ \ \ \ \ \ other agent offers good n+q \\
\ \ \ \ \ \ \ \ \ \ WHILE trade increases both agents' utilities: \\ 
\ \ \ \ \ \ \ \ \ \ \ \ trade one good n for one good n+q
\end{verbatim}

## Uniform activation of pairwise combinations (A=5)
...
\node[box] (D) at (9,0) {$a_4$};
\node[box] (E) at (12,0) {$a_5$};
...
\draw[->] (B) to[bend right=35] (D);
\draw[->] (B) to[bend right=45] (E);
...
\pause 3. Shuffle list of indices:
$$
\begin{bmatrix}
